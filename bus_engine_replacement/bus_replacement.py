...
counts = np.bincount(
    usage_valid.astype(int), minlength=max(int(np.max(usage_valid)) + 1, 6)
)
...
def compute_policy(v, theta):
    ...
    # log_pi = q_both + logsumexp(q_both, axis=1)
    return jax.nn.softmax(q_both, axis=1)
...
def log_likelihood(theta):
    ...
    return jnp.sum(jnp.log(probs_observed))
...
optimizer = optax.chain(optax.clip_by_global_norm(10.0), optax.adam(0.01))
...
for i in range(n_steps):
    ...
    if i % 50 == 0:
        print(
            f"  Step {i:3d}: -log L(θ)={loss:10.2f}, θ=[{theta[0]:.2f}, {theta[1]:.2f}]"
        )

# Show final
print(
    f"  Step {n_steps - 1:3d}: -log L(θ)={losses[-1]:10.2f}, θ=[{theta_history[-1][0]:.2f}, {theta_history[-1][1]:.2f}]"
)
...
print(
    f"  Error: ΔRC={abs(theta_hat[0] - 9.75):.2f}, ΔC1={abs(theta_hat[1] - 2.633):.3f}"
)
...
ax.text(
    ...
    bbox=dict(
        boxstyle="round",
        facecolor="lightgreen" if (methods_match and converged) else "yellow",
        alpha=0.5,
    ),
)
...
ax.scatter(
    [len(losses) - 1], [final_loss], s=100, color="green", zorder=10, label="End"
)
...
ax.text(
    0.02,
    0.02,
    f"Improving at ~{rate:.2f} per step",
    transform=ax.transAxes,
    fontsize=10,
    ha="left",
    va="bottom",
