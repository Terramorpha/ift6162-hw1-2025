...
class LinearSplineBasis(BasisFunction):
    ...
    def __call__(self, x: np.ndarray, i: int) -> np.ndarray:
        ...
        if i > 0:
            ...
            result[mask] = (x[mask] - self.nodes[i - 1]) / (
        ...
        if i < self.n_nodes - 1:
            ...
            result[mask] = (self.nodes[i + 1] - x[mask]) / (
...
class TestFunction(ABC):
    ...
    @abstractmethod
    def apply(
        self, residual_func: Callable, coeffs: np.ndarray, domain: Tuple[float, float]
    ) -> np.ndarray:
...
class GalerkinTest(TestFunction):
    ...
    def apply(
        ...
        nodes, weights = np.polynomial.legendre.leggauss(len(coeffs))

        w_quad = weights / (1 - -1) * (b - a)
        x_quad = (nodes - -1) / (1 - -1) * (b - a) + a

        Rx = residual_func(x_quad, coeffs)

        return np.array(
            [np.sum(Rx * self.basis(x_quad, i) * w_quad) for i in range(len(coeffs))],
        )
...
class OperatorEquation(ABC):
    ...
    @abstractmethod
    def residual(
        self, x: np.ndarray, coeffs: np.ndarray, basis: BasisFunction
    ) -> np.ndarray:
...
class ProjectionMethod:
    ...
    def _solve_iterate(
        self, a0: np.ndarray, max_iter: int, tol: float, verbose: bool
    ) -> np.ndarray:
    ...
    def plot_solution(
        self, coeffs: np.ndarray, n_plot: int = 200, title: str = "Solution"
    ):
