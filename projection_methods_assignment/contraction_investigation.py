...
class ContractionAnalyzer:
    ...
    def __init__(
        self, bellman_eq: BellmanEquation, basis: BasisFunction, nodes: np.ndarray
    ):
        """Initialize analyzer
    ...
    def apply_discretized_bellman(self, coeffs: np.ndarray) -> np.ndarray:
        ...
        Phi = np.array([self.basis(self.nodes, i) for i in range(len(coeffs))]).T
    ...
    def estimate_lipschitz_constant(
        ...
        distance_fn = (
            self.compute_distance_linf if norm == "linf" else self.compute_distance_l2
        )
        ...
        ratios = [sample() for _ in range(n_samples)]
    ...
    def visualize_operator_behavior(self, coeffs1: np.ndarray, coeffs2: np.ndarray):
        ...
        ax.plot(
            self.nodes,
            self.basis.evaluate_approximation(self.nodes, coeffs1),
            "bo",
            markersize=4,
            alpha=0.5,
        )
        ax.plot(
            self.nodes,
            self.basis.evaluate_approximation(self.nodes, coeffs2),
            "ro",
            markersize=4,
            alpha=0.5,
        )
        ...
        ax.plot(
            self.nodes,
            self.basis.evaluate_approximation(self.nodes, Ta1),
            "bo",
            markersize=4,
            alpha=0.5,
        )
        ax.plot(
            self.nodes,
            self.basis.evaluate_approximation(self.nodes, Ta2),
            "ro",
            markersize=4,
            alpha=0.5,
        )
        ...
        ax.axhline(
            self.bellman.delta * dist_input_linf,
            color="r",
            linestyle="--",
            alpha=0.5,
            label=f"γ·||v1 - v2||∞ = {self.bellman.delta * dist_input_linf:.3f}",
        )
        ax.set_xlabel("State (s)")
        ...
        plt.suptitle(
            f"Contraction Analysis: {type(self.basis).__name__}\n"
...
def compare_basis_functions():
    ...
    for idx, (name, result) in enumerate(results.items()):
        ...
        ax.axvline(
            gamma, color="red", linestyle="--", linewidth=2, label=f"γ = {gamma:.2f}"
        )
        ax.axvline(
            result["lipschitz_linf"],
            color="green",
            linestyle="-",
            linewidth=2,
    ...
    plt.suptitle(
        "Distribution of Lipschitz Ratios Across Different Bases",
        fontsize=14,
        fontweight="bold",
    )
    plt.tight_layout()
