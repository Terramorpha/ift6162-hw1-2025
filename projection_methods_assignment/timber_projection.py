...
class BellmanEquation(OperatorEquation):
    ...
    def residual(
        self, s: np.ndarray, coeffs: np.ndarray, basis: BasisFunction
    ) -> np.ndarray:
...
class BellmanProjectionMethod(ProjectionMethod):
    ...
    def _solve_iterate(
        ...
        for _ in range(max_iter):
            ...
            v_func = lambda s: self.basis.evaluate_approximation(s, a_k)
            ...
            Phi = np.array([self.basis(nodes, i) for i in range(len(nodes))]).T
            ...
            if np.linalg.norm(a_new - a_k) < tol:
                a_k = a_new
                break
            else:
                a_k = a_new
...
def solve_timber_with_projection(
    n_nodes: int = 50,
    basis_type: str = "linear",
